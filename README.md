...
const serveImage = registerServe({
  ...
  // Allowed remote hosts for fetching network images. Each entry is an exact
  // hostname, OR a `*.domain` wildcard that matches the apex and any
  // subdomain — use a wildcard for services that redirect to a CDN subdomain
  // (e.g. picsum.photos → fastly.picsum.photos).
  allowedNetworkList: ["cdn.example.com", "*.picsum.photos"],
...
| `allowedNetworkList` | `string[]`                                | `[]`               | Allowed remote hosts. Others immediately fall back. Each entry is either an **exact hostname** or a **`*.domain` wildcard** that matches the apex and any subdomain (`*.picsum.photos` matches `picsum.photos`, `fastly.picsum.photos`, `a.b.picsum.photos`). A wildcard must have at least two labels after `*.` (so `*.com` is rejected at `registerServe()`) and public-suffix families (`*.co.uk`) are not special-cased. Use a wildcard for services that redirect to a CDN subdomain. The wildcard relaxes only the hostname allowlist — every redirect hop is still re-validated against the public-IP DNS guard, so it can never reach a private address. **Entries are trimmed and lowercased at schema-parse time**, so `["CDN.Example.com"]` matches a request URL whose hostname the WHATWG URL parser has lowercased to `cdn.example.com`. |
| `cacheControl`       | `string`                                  | `undefined`        | Cache-Control header value                                              |
| `etag`               | `boolean`                                 | `true`             | Emit ETag and honor If-None-Match                                       |
| `minWidth`           | `number`                                  | `50`               | Minimum accepted width. May be `>= 1` (the framework's hard floor) — lower it below 50 to serve small images (avatars, icons, favicons). Enforced at `registerServe()`. |
| `maxWidth`           | `number`                                  | `4000`             | Maximum accepted width. Must be `<= 4000` — the framework's hard ceiling, enforced at `registerServe()` |
| `minHeight`          | `number`                                  | `50`               | Minimum accepted height. May be `>= 1` (the framework's hard floor) — lower it below 50 to serve small images. Enforced at `registerServe()`. |
...
| `width`   | `number`                | `undefined` | Desired output width (px). Validated against the framework's hard `[1, 4000]` window — out-of-window requests return a fallback image rather than being resized — then clamped to the configured `minWidth`/`maxWidth`. |
| `height`  | `number`                | `undefined` | Desired output height (px). Validated against the framework's hard `[1, 4000]` window — out-of-window requests return a fallback image rather than being resized — then clamped to the configured `minHeight`/`maxHeight`. |
...
- Only fetches from explicitly allowed domains (`allowedNetworkList`). Each entry is an exact hostname or a `*.domain` wildcard (matches the apex + any subdomain; must have ≥2 labels after `*.`; public-suffix families are not special-cased). The wildcard relaxes only the hostname allowlist — the public-IP DNS guard below still runs on every hop. Allowlist entries are normalised (trimmed + lowercased) at schema-parse time so the case-insensitive matching contract is enforced regardless of how the option was supplied (env file, JSON config, etc.).
...
- **DNS rebinding mitigation (pinned `lookup`).** Every hop resolves the destination hostname **once** via `dns.lookup`, validates that **every** resolved address is public (if any is private the whole host is rejected), then passes axios a per-request `httpAgent`/`httpsAgent` whose `lookup` function is pinned to those validated addresses. The TCP socket is therefore guaranteed to connect only to an IP the framework validated, rather than whatever the kernel resolver returns microseconds later. This closes the classic DNS-rebinding TOCTOU window where an attacker-controlled authoritative server answers the validation lookup with a public IP and the subsequent connect-time lookup with `127.0.0.1` / `169.254.169.254`. Each redirect hop re-resolves and re-pins so chained rebinding attempts are also defeated. The agents leave Node's default Happy-Eyeballs (`autoSelectFamily`) enabled so the connect can fail over across the validated addresses (e.g. try IPv4 when a resolved-but-unreachable IPv6 is returned first) instead of hanging until timeout — the pinned `lookup` remains the sole SSRF boundary, so this failover can only ever reach an address that was already validated public.
...
- `resolvePinnedAddresses(hostname: string): Promise<Array<{ address: string, family: 4 | 6 }> | null>` — Resolves a hostname once and returns **every** validated `{ address, family }` pair so a subsequent socket connection can be pinned to the exact IPs the validator approved (DNS-rebinding mitigation) while still allowing Happy-Eyeballs to fail over across them. Returns `null` when the host is empty, resolves to no addresses, resolves to (or is) a private/loopback/link-local IP (if **any** resolved address is private the whole host is rejected), or DNS resolution fails.
- `resolvePinnedAddress(hostname: string): Promise<{ address: string, family: 4 | 6 } | null>` — Backward-compatible single-address variant: returns the **first** validated pair (or `null`). Prefer `resolvePinnedAddresses` so failover across all validated addresses is possible.
- `buildPinnedAgents(address: string, family: 4 | 6): { httpAgent, httpsAgent }` (also accepts a pre-validated `Array<{ address, family }>`) — Builds `http.Agent` and `https.Agent` instances whose `lookup` function is pinned to the supplied address(es). The agents deliberately leave Node's default Happy-Eyeballs enabled (no `autoSelectFamily: false`, no single pinned `family`) so the connect can fail over across the pinned addresses; the pinned `lookup` remains the sole SSRF boundary. Drop them into axios / fetch to guarantee the TCP socket connects only to a validated IP.
